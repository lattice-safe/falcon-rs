...
use crate::shake::{InnerShake256Context, i_shake256_init, i_shake256_inject, i_shake256_flip, i_shake256_extract};
use crate::codec;
use crate::vrfy;
use crate::sign;
use crate::common;
...
fn align_u64(tmp: &mut [u8]) -> &mut [u8] {
    ...
    let off = addr & 7;
    if off != 0 { &mut tmp[8 - off..] } else { tmp }
}

fn align_u16(tmp: &mut [u8]) -> &mut [u8] {
    let addr = tmp.as_ptr() as usize;
    if addr & 1 != 0 { &mut tmp[1..] } else { tmp }
}
...
pub fn falcon_keygen_make(
    ...
    let mut u: usize = 1;
    {
        ...
        let v = codec::trim_i8_encode(
            Some(&mut privkey[u..]),
            f, logn, codec::MAX_FG_BITS[logn as usize] as u32,
        );
        if v == 0 { return FALCON_ERR_INTERNAL; }
        u += v;
    }
    {
        ...
        let v = codec::trim_i8_encode(
            Some(&mut privkey[u..]),
            g, logn, codec::MAX_FG_BITS[logn as usize] as u32,
        );
        if v == 0 { return FALCON_ERR_INTERNAL; }
        u += v;
    }
    {
        let big_f = unsafe { core::slice::from_raw_parts(tmp[big_f_off..].as_ptr() as *const i8, n) };
        let v = codec::trim_i8_encode(
            Some(&mut privkey[u..]),
            big_f, logn, codec::MAX_FG_BITS_UPPER[logn as usize] as u32,
        );
        if v == 0 { return FALCON_ERR_INTERNAL; }
        u += v;
    }
...
pub fn falcon_make_public(
    ...
    if privkey.is_empty() { return FALCON_ERR_FORMAT; }
    if privkey[0] & 0xF0 != 0x50 { return FALCON_ERR_FORMAT; }
    let logn = (privkey[0] & 0x0F) as u32;
    if logn < 1 || logn > 10 { return FALCON_ERR_FORMAT; }
    if privkey.len() != falcon_privkey_size(logn) { return FALCON_ERR_FORMAT; }
    let pk_len = falcon_pubkey_size(logn);
    if pubkey.len() < pk_len { return FALCON_ERR_SIZE; }
    if tmp.len() < falcon_tmpsize_makepub(logn) { return FALCON_ERR_SIZE; }
    ...
    let mut u: usize = 1;
    let v = codec::trim_i8_decode(f, logn, codec::MAX_FG_BITS[logn as usize] as u32,
        &privkey[u..]);
    if v == 0 { return FALCON_ERR_FORMAT; }
    u += v;
    let v = codec::trim_i8_decode(g, logn, codec::MAX_FG_BITS[logn as usize] as u32,
        &privkey[u..]);
    if v == 0 { return FALCON_ERR_FORMAT; }

    // Compute public key.
    let h_off = if (unsafe { ptr.add(2 * n) as usize }) & 1 != 0 { 2 * n + 1 } else { 2 * n };
    let h = unsafe { core::slice::from_raw_parts_mut(ptr.add(h_off) as *mut u16, n) };
    let atmp = unsafe {
        core::slice::from_raw_parts_mut(ptr.add(h_off + 2 * n), tmp.len() - h_off - 2 * n)
    };
    ...
    if v != pk_len - 1 { return FALCON_ERR_INTERNAL; }
    0
}

/// Get the Falcon degree from a header byte.
pub fn falcon_get_logn(obj: &[u8]) -> i32 {
    if obj.is_empty() { return FALCON_ERR_FORMAT; }
...
pub fn falcon_sign_dyn_finish(
    ...
    if logn < 1 || logn > 10 { return FALCON_ERR_FORMAT; }
    if privkey.len() != falcon_privkey_size(logn) { return FALCON_ERR_FORMAT; }
    if tmp.len() < falcon_tmpsize_signdyn(logn) { return FALCON_ERR_SIZE; }
    let es_len = *sig_len;
    if es_len < 41 { return FALCON_ERR_SIZE; }

    match sig_type {
        FALCON_SIG_COMPRESSED => {},
        FALCON_SIG_PADDED => {
            if es_len < falcon_sig_padded_size(logn) { return FALCON_ERR_SIZE; }
        },
        FALCON_SIG_CT => {
            if es_len < falcon_sig_ct_size(logn) { return FALCON_ERR_SIZE; }
        },
    ...
    let mut u: usize = 1;
    let v = codec::trim_i8_decode(f, logn, codec::MAX_FG_BITS[logn as usize] as u32, &privkey[u..]);
    if v == 0 { return FALCON_ERR_FORMAT; }
    u += v;
    let v = codec::trim_i8_decode(g, logn, codec::MAX_FG_BITS[logn as usize] as u32, &privkey[u..]);
    if v == 0 { return FALCON_ERR_FORMAT; }
    u += v;
    let v = codec::trim_i8_decode(big_f, logn, codec::MAX_FG_BITS_UPPER[logn as usize] as u32, &privkey[u..]);
    if v == 0 { return FALCON_ERR_FORMAT; }
    u += v;
    if u != privkey.len() { return FALCON_ERR_FORMAT; }
    ...
    loop {
        *hash_data = sav_hash_data.clone();
        ...
        let sv: &mut [i16] = unsafe { core::slice::from_raw_parts_mut(ptr.add(hm_off) as *mut i16, n) };
        ...
        let atmp_full = unsafe { core::slice::from_raw_parts_mut(ptr.add(atmp_off), tmp.len() - atmp_off) };
        ...
        match sig_type {
            ...
            FALCON_SIG_CT => {
                ...
                let v = codec::trim_i16_encode(
                    Some(&mut sig[u_sig..]),
                    sv, logn, codec::MAX_SIG_BITS[logn as usize] as u32,
                );
                if v == 0 { return FALCON_ERR_SIZE; }
                *sig_len = u_sig + v;
                return 0;
...
pub fn falcon_sign_tree_finish(
    ...
    if expanded_key.is_empty() { return FALCON_ERR_FORMAT; }
    let logn = expanded_key[0] as u32;
    if logn < 1 || logn > 10 { return FALCON_ERR_FORMAT; }
    if tmp.len() < falcon_tmpsize_signtree(logn) { return FALCON_ERR_SIZE; }
    let es_len = *sig_len;
    if es_len < 41 { return FALCON_ERR_SIZE; }
    ...
    let ek_off = if ek_addr & 7 != 0 { 8 - (ek_addr & 7) } else { 0 };
...
pub fn falcon_sign_dyn(
    ...
    falcon_sign_dyn_finish(rng, sig, sig_len, sig_type,
        privkey, &mut hd, &nonce, tmp)
}
...
pub fn falcon_sign_tree(
    ...
    if r != 0 { return r; }
    shake256_inject(&mut hd, data);
    falcon_sign_tree_finish(rng, sig, sig_len, sig_type,
        expanded_key, &mut hd, &nonce, tmp)
}
...
pub fn falcon_expand_privkey(
    ...
    if logn < 1 || logn > 10 { return FALCON_ERR_FORMAT; }
    if privkey.len() != falcon_privkey_size(logn) { return FALCON_ERR_FORMAT; }
    if expanded_key.len() < falcon_expandedkey_size(logn) { return FALCON_ERR_SIZE; }
    if tmp.len() < falcon_tmpsize_expandpriv(logn) { return FALCON_ERR_SIZE; }
...
pub fn falcon_verify_start(
    ...
    if sig.len() < 41 { return FALCON_ERR_FORMAT; }
    shake256_init(hash_data);
...
pub fn falcon_verify_finish(
    ...
    if sig.len() < 41 || pubkey.is_empty() { return FALCON_ERR_FORMAT; }
    if pubkey[0] & 0xF0 != 0x00 { return FALCON_ERR_FORMAT; }
    let logn = (pubkey[0] & 0x0F) as u32;
    if logn < 1 || logn > 10 { return FALCON_ERR_FORMAT; }
    if sig[0] & 0x0F != logn as u8 { return FALCON_ERR_BADSIG; }
    ...
    match sig_type {
        0 => {
            match sig[0] & 0xF0 {
                0x30 => {},
                0x50 => {
                    if sig.len() != falcon_sig_ct_size(logn) { return FALCON_ERR_FORMAT; }
                    ct = true;
                },
        ...
        FALCON_SIG_COMPRESSED => {
            if sig[0] & 0xF0 != 0x30 { return FALCON_ERR_FORMAT; }
        },
        FALCON_SIG_PADDED => {
            if sig[0] & 0xF0 != 0x30 { return FALCON_ERR_FORMAT; }
            if sig.len() != falcon_sig_padded_size(logn) { return FALCON_ERR_FORMAT; }
        },
        FALCON_SIG_CT => {
            if sig[0] & 0xF0 != 0x50 { return FALCON_ERR_FORMAT; }
            if sig.len() != falcon_sig_ct_size(logn) { return FALCON_ERR_FORMAT; }
            ct = true;
        },
        _ => return FALCON_ERR_BADARG,
    ...
    if pubkey.len() != falcon_pubkey_size(logn) { return FALCON_ERR_FORMAT; }
    if tmp.len() < falcon_tmpsize_verify(logn) { return FALCON_ERR_SIZE; }

    let n: usize = 1 << logn;
    let ptr = tmp.as_mut_ptr();

    // Align for u16.
    let base_off = if (ptr as usize) & 1 != 0 { 1usize } else { 0usize };
    ...
    let u_sig: usize = 41;
    let v = if ct {
        codec::trim_i16_decode(sv, logn, codec::MAX_SIG_BITS[logn as usize] as u32, &sig[u_sig..])
    } else {
    ...
    if u_sig + v != sig.len() {
            ...
            || sig_type == FALCON_SIG_PADDED
        {
            let mut pos = u_sig + v;
            while pos < sig.len() {
                if sig[pos] != 0 { return FALCON_ERR_FORMAT; }
                pos += 1;
            }
        } else {
            return FALCON_ERR_FORMAT;
