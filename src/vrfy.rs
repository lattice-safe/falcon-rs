...
fn mq_poly_montymul_ntt(f: &mut [u16], g: &[u16], logn: u32) {
    ...
    for u in 0..n {
        ...
        unsafe {
            *f.get_unchecked_mut(u) = mq_montymul(
                *f.get_unchecked(u) as u32,
                *g.get_unchecked(u) as u32,
            ) as u16;
        }
    }
}
...
pub fn verify_raw(
    ...
    let s1: &mut [i16] = unsafe {
        core::slice::from_raw_parts_mut(tt.as_mut_ptr() as *mut i16, n)
    };
    for u in 0..n {
...
pub fn complete_private(
    ...
    let n: usize = 1 << logn;
    let tt: &mut [u16] = unsafe {
        core::slice::from_raw_parts_mut(tmp.as_mut_ptr() as *mut u16, 2 * n)
    };
